function [ss,gg,tt,ff,zo]=specsubtrac(si,fsz,wtype,pp)
...
%   wtype   windows type ex: hamming,hanning,rectangular
...
if ~exist('wtype','var')
    w=sqrt(windows('hamming',nf+1)')';
else
    switch wtype
        case 'hanning'
            w=sqrt(windows(wtype,nf+1)')'; 
        case 'cos'
            w=sqrt(windows(wtype,nf+1)')'; 
        case 'dolph'
            w=sqrt(windows(wtype,nf+1)')'; 
